  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="relative rounded-full p-2 text-[#94a3b8] hover:bg-[#334155] hover:text-white focus:outline-none focus:ring-2 focus:ring-[#8b5cf6]"
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-lg border border-[#334155] bg-[#1e293b] shadow-xl">
          {/* Header */}
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 && (
                <button
                  ...
                  className="flex items-center gap-1 rounded px-2 py-1 text-xs text-[#8b5cf6] hover:bg-[#334155]"
                >
              ...
              <button
                ...
                className="rounded p-1 text-[#64748b] hover:bg-[#334155] hover:text-white"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
          ...
          <div className="max-h-96 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="py-8 text-center">
                <Bell className="mx-auto mb-2 h-8 w-8 text-[#475569]" />
              ...
              notifications.map(notification => (
                <button
                  ...
                  className={`flex w-full items-start gap-3 border-b border-[#334155] px-4 py-3 text-left transition-colors hover:bg-[#334155] ${
                    !notification.read ? 'bg-[#8b5cf6]/10' : ''
                  }`}
                >
                  {/* Icon */}
                  <div className={`mt-0.5 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full ${
                    !notification.read ? 'bg-[#8b5cf6]/20' : 'bg-[#334155]'
                  }`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`text-sm ${!notification.read ? 'font-medium text-white' : 'text-[#94a3b8]'}`}>
                      {notification.title}
                    </p>
                  ...
                  {/* Unread indicator */}
                  {!notification.read && (
                    <div className="mt-2 h-2 w-2 flex-shrink-0 rounded-full bg-[#8b5cf6]" />
                  )}
                </button>
              ))
            )}
          ...
          {/* Footer */}
          {notifications.length > 0 && (
            <div className="border-t border-[#334155] px-4 py-2">
              <button
                onClick={() => {
                  setIsOpen(false);
                  router.push('/notifications');
                }}
                className="w-full rounded py-2 text-center text-sm text-[#8b5cf6] hover:bg-[#334155]"
              >
